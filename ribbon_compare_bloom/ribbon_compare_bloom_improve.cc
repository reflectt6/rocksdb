...
#include "ribbon_compare_bloom_improve.h"

using namespace RIBBON_COMPILE_BLOOM;

// 测试用例名称
class Compare_Improve : public ::testing::Test {};

// HBase UT CellGen
TEST(Compare_Improve, TestHBaseBloomChunk) {
  ...
  std::cout << "bloom chunk size = " << chunk.byteSize / 1000 << "kb"
            << std::endl;
...
// generate cell by Standard KeyGen
TEST(Compare_Improve, TestHBaseBloomChunkSKG) {
  using TypeParam = DefaultTypesAndSettings;
  ...
  StandardKeyGen stop(added_str, chunk.maxKeys);
  ...
  while (cur != other_keys_end) {
    ...
    Cell* cell = new Cell(s, s.size());
    timer.Start();
    bool b = chunk.tmpQuery(*cell);
    queryDuration += timer.ElapsedNanos();
    if (b) {
      fp_count++;
    }
    ++cur;
  ...
  while (cur != stop) {
    ...
    timer.Start();
    bool b = chunk.tmpQuery(*cell);
    queryDuration2 += timer.ElapsedNanos();
    if (!b) {
      fp_count2++;
    }
    ++cur;
  ...
  std::cout << "bloom chunk size = " << chunk.byteSize / 1000 << "kb"
            << std::endl;
  std::cout << "save keys = " << chunk.keyCount << std::endl;
  std::cout << "expected fp rate = " << FLAGS_errorRate << std::endl;
  std::cout << "construction time = " << duration / 1000 / 1000 << "ms"
            << std::endl;
  std::cout << "overhead rate = " << 1.0 * chunk.byteSize / chunk.maxKeys
            << std::endl;
  std::cout << "fp rate = " << 1.0 * fp_count / (FLAGS_max_check) << std::endl;
  std::cout << "cnt = " << cnt << std::endl;
  std::cout << "fp_count = " << fp_count << std::endl;
  std::cout << "查询已插入的key，返回true的数量 = " << fp_count2 << std::endl;
  std::cout << "query time = " << queryDuration / 1000 / 1000 << "ms"
            << std::endl;
  std::cout << "查询已插入的key的时间 = " << queryDuration2 / 1000 / 1000
            << "ms" << std::endl;
...
TEST(CompareTest, RepeatCompactnessAndBacktrackAndFpRate) {
  ...
  for (ConstructionFailureChance cs : TypeParam::FailureChanceToTest()) {
    ...
    for (uint32_t i = 0; i < FLAGS_thoroughness;
         ++i) {  // 不太懂这个迭代次数有啥用
      ...
      --num_to_add;
      for (;;) {  // 很迷惑，反正通过一些操作，把上面的num_slots重新给了一个较小的值
                  // 具体实现在ribbon_config.cc
      ...
      bool batch_success;
      {
        ...
        if (TypeParam::kHomogeneous || overhead_ratio < 1.01 ||
          ...
          other_keys_too_big_end +=
              num_to_add;  // 一般这个num_to_add是大于nums_slots的一半的，所以这里再添加num_to_add一定会超过filter容量
          banding.EnsureBacktrackSize(std::max(num_to_add, batch_size));
          EXPECT_FALSE(banding.AddRangeOrRollBack(
              other_keys_begin,
        ...
        std::cout << "测试backtracking的时间："
                  << timeStub.ElapsedNanos(true) / 1000 / 1000
                  << "ms，是否被跳过："
                  << (TypeParam::kHomogeneous || overhead_ratio < 1.01 ||
                      batch_size == 0)
                  << std::endl;

        // Also verify that redundant adds are OK (no effect)
        //        ASSERT_TRUE(
        //            banding.AddRange(keys_begin, KeyGen(addedKeyPrefix +
        //            num_to_add / 8)));
        ASSERT_TRUE(
        ...
        EXPECT_LE(
            reseeds,
            8 + log2_thoroughness);  // 迷惑，其实干嘛要做log运算我是很迷惑的
      ...
      std::cout << "对比bloom的时间："
                << timeStub.ElapsedNanos(true) / 1000 / 1000 << "ms， 此时第"
                << i << "次迭代结束" << std::endl;
    }
    ...
    if (TypeParam::kHomogeneous) {
      EXPECT_EQ(
          total_reseeds,
          0U);  // 说明kHomogeneous在banding中不会失败，因此不会改变种子的值
    } else {
    ...
    std::cout << "测试 reseed 是否符合预期的时间（测试逻辑不太懂）："
    ...
    std::cout << "计算expand、single、batch、avg fp "
                 "rate是否符合预期的时间（应该没啥计算量） ："
              << timeStub.ElapsedNanos(true) / 1000 / 1000 << "ms， 此时enum["
              << cs << "]结束" << std::endl;
  }
}
...
TEST(CompareTest, RepeatCompactnessAndBacktrackAndFpRate2) {
  ...
  // 只测一种reseed失败率
  ConstructionFailureChance cs = TypeParam::FailureChanceToTest()[0];
  double expected_reseeds;
  switch (cs) {
    default:
      assert(false);
  ...
  for (uint32_t i = 0; i < FLAGS_thoroughness;++i) {  // 不太懂这个迭代次数有啥用
    ...
    bool batch_success;
    {
      ...
      std::cout << "banding的时间："
                ...
                << "ms， 此时添加了num_to_add个key：" << num_to_add
                << "banding占据数量" << occupied_count << std::endl;
      if (TypeParam::kHomogeneous || overhead_ratio < 1.01 || batch_size == 0) {
        // 可以看出来backtracking是用来记录banding失败的
        // Homogeneous not compatible with backtracking because add
        // 在开销小于1.01时，我们不应该要求更多
        // doesn't fail. Small overhead ratio too packed to expect more
    ...
    std::cout << "对比bloom的时间："
              << timeStub.ElapsedNanos(true) / 1000 / 1000 << "ms， 此时第" << i
              << "次迭代结束" << std::endl;
  }
  ...
  if (TypeParam::kHomogeneous) {
    ...
    // success rate (average of 1.0 re-seeds) or slightly better. But 1.15 is
    // also close enough.
    EXPECT_LE(total_reseeds, InfrequentPoissonUpperBound(
                                 1.15 * expected_reseeds * FLAGS_thoroughness));
    // Would use 0.85 here instead of 0.75, but
    // TypesAndSettings_Hash32_SmallKeyGen can "beat the odds" because of
    // sequential keys with a small, cheap hash function. We accept that
    // there are surely inputs that are somewhat bad for this setup, but
    ...
    EXPECT_GE(total_reseeds, InfrequentPoissonLowerBound(
                                 0.75 * expected_reseeds * FLAGS_thoroughness));
  }
...
// 自定义参数 但是不使用rangeAdd 而是一个一个的add
TEST(CompareTest, RepeatCompactnessAndBacktrackAndFpRate3) {
  using TypeParam = Settings_Coeff128_Homog;
  ...
  for (uint32_t i = 0; i < FLAGS_thoroughness;++i) {  // 不太懂这个迭代次数有啥用
    ...
    bool batch_success;
    {
      Banding banding;
      // Traditional solve for a fixed set.
      KeyGen tmp_begin(added_str, 0);
      KeyGen tmp_end(added_str, 1);
      ASSERT_TRUE(
          banding.ResetAndFindSeedToSolve(num_slots, tmp_begin, tmp_end));
      for (uint32_t j = 1; j < num_to_add; ++j) {
        KeyGen tmp_begin2(added_str, j);
        KeyGen tmp_end2(added_str, j + 1);
        ASSERT_TRUE(
            banding.AddRange(tmp_begin2, tmp_end2));
      }
//      ASSERT_TRUE(
//          banding.ResetAndFindSeedToSolve(num_slots, keys_begin, keys_end));
...
// 两种soln分开计时 自定义参数 但是不使用rangeAdd 而是一个一个的add
TEST(CompareTest, RepeatCompactnessAndBacktrackAndFpRate4) {
  using TypeParam = Settings_Coeff128_Homog;
